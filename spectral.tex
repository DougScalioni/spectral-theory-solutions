...
\begin{exercise}{1.5.4}
    Let $T : \banach \hookleftarrow$ be linear with $\dim \banach < \infty$. Show that $\sigma(T)$ is the
    set of eigenvalues of $T$ and, by the fundamental theorem of algebra, conclude that
    $\sigma(T ) \neq \emptyset$ in this case.

    A linear operator $S$ is invertible iff $\kernel(S) = \left\{0\right\}$. Thus, if $S\xi = (T - \lambda \Id)\xi = 0$,
    the non zero values for $\xi$, for which the operator $S$ is \textbf{not} invertible (for some $\lambda$), are exactly the set of eigenvectors (and the corresponding eigenvalues).

    The tricky part here is that for finite spaces, the inverse of an operator (if it exists) is automatically bounded, which may not happen in infinite dimension.
    In sum, in infinite dimensions, there might be certain values of $\lambda$ that are not eigenvalues, so the inverse of $S$ exists but isn't bounded, and $\lambda$
    is in the spectrum. The spectrum, then, contains the set of eigenvalues.

    In finite spaces the set of eigenvalues is the set of solutions of the characteristic equation, which is garanteed not to be empty by the Fundamental Theorem of Algebra.
...
\begin{exercise}{1.5.5}
    ...
    %%    $$ \beta_1 (\lambda_1\xi_1-\lambda_{n+1}\xi_1) + \dots + \beta_n (\lambda_n\xi_n-\lambda_{n+1}\xi_n) = \beta_{n+1} (T\xi_{n+1} - \lambda_{n+1}\xi_{n+1})$$
