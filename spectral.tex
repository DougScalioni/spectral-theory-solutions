...
\begin{exercise}{1.2.16}
    If $\normed \subset \banach$, show that $T \in \bounded(\normed , \banach)$ is closed if, and only if, $N$ is a Banach space.
...
\begin{exercise}{1.2.24}
    ...
    Then we can define a linear transformation by relating the elements of the basis of $\pi_{1}(E)$ and $\pi_{2}(E)$ (as long as $\dim \pi_1 > \dim \pi_2$, right? CHECK THIS).
